...
import { formatDecimal, formatNumber } from "~/utils/format/number";
...
export const loader = async () => {
  ...
  try {
    res = (await Promise.all([
      ...
      getGasFee(),
      activity().then((res) => {
        res.entries = res.entries.reduce((curr, next) => {
          if (
            !curr.length ||
            new Date(next.event.end_at).valueOf() >
            new Date(curr[0].event.end_at).valueOf()
          ) {
            curr.unshift(next);
          } else {
            curr.push(next);
      ...
      daily({
        ...
        return {
          personRelease: personData.map((item) =>
            formatNumber(formatUnits(item.countStr as bigint, 18), 2)
          ),
          days: personData.map((item) => item.statDate),
          addressCount: personData.map((item) => item.addressCount),
        };
      }),
    ])) as any;
  } catch (error) { }
  ...
  return (
    <Stack gap={{ base: "16px", md: "30px" }} p={{ base: "!0", md: "!22px" }}>
      ...
      >
        ...
        />
        <TokenRelease releaseData={releaseData} />
