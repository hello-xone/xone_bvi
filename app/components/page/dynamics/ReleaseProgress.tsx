...
export default function ReleaseProgress() {
  ...
  const getEpochDetailsInfo = async () => {
    ...
    const currentIndex = res.details.findIndex(
      (item) => item.epoch === res.curlEpoch
    );

    setActiveIndex(currentIndex >= 0 ? currentIndex : 0);
    setTimeout(() => {
      scrollToCenter(currentIndex >= 0 ? currentIndex : 0);
    }, 100);
  };
  ...
  const dotStyles = {
    released: "w-[26px] min-w-[26px] h-[26px] bg-[url('/images/icons/end.svg')] bg-no-repeat bg-fixed bg-cover !bottom-[-5px]",
    releasing: "w-[26px] min-w-[26px] h-[26px] border-[3px] border-[#f00] bg-white !bottom-[-5px]",
  ...
  return (
    ...
    >
      ...
      <Box
        pos="absolute"
        right={{ base: "17px", md: "117px" }}
        top={{ base: "-60px", md: "15px" }}
      >
        <ReactSVG
...
function ReleaseProgressInfo() {
  ...
  return (
    <Wrap
      ...
      mx={{ base: "16px", md: "68px" }}
      my={{ base: "20px", md: "40px" }}
      px={{ base: 0, md: "92px" }}
      py={{ base: 0, md: "29px" }}
      bg={{ base: "none", md: "#FFF0F0" }}
      border={{ base: "none", md: "1px solid #FF0206" }}
      rounded="20px"
      gap={{ base: "20px", md: 0 }}
    >
      {config.map((item, index) => (
        <>
          <Flex
            ...
            w={{ base: "100%", md: "auto" }}
            p={{ base: "20px", md: 0 }}
            bg={{ base: "#FFF0F0", md: "none" }}
            border={{ base: "1px solid #FF0206", md: "none" }}
            rounded={{ base: "20px", md: 0 }}
          >
            <Box
