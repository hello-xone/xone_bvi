...
export default function Data({ children, no, title }: Props) {
  ...
  useEffect(() => {
    if (no) {
      send({
        no,
        type,
        contrast: dataTime ? parseInt(dataTime) : undefined,
  ...
  const cardConfig = [
    ...
    },
    {
      ...
      value: formatNumber(totalData("gv"), 2),
    ...
    >
      <Wrap gap={{ base: "12px", md: "20px" }} alignItems="center">
        ...
        >
          {title}
          {isMobile && (
            <TimeSelect paramKey="dataTime" onChange={setDataTime} />
          )}
        </Font>
