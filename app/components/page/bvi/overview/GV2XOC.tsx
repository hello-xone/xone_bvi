  ...
  return (
    ...
    >
      ...
      <Flex
        ...
        lineHeight="normal"
      >
        {item.byte ?? true ? <FormatByte value={+item.value} /> : item.value}
        {item.isView && (
...
export default function GV2XOC() {
  ...
  const { data: convertListData, send: chartSend } = useRequest(convertList, {
    immediate: false,
    initialData: { list: [] },
  });

  const chartData = (convertListData as any)?.list?.reduce?.(
    (curr, next) => {
      curr.days.push(next.day);
  ...
  const gvRecordConfig: DataItem[] = [
    {
      ...
      tip: t("Total GV gained in all seasons"),
      percentage: rateCalc(gv_rate),
      direction: incOrDec(gv_rate),
      value: gv,
    },
  ...
  const selectedGvData = gvRelease.no ? [gvRelease] : selectedGv;
